...
class BeyondMimic(FSMState):
    def __init__(self, state_cmd:StateAndCmd, policy_output:PolicyOutput):
        ...
        self.motion_step = 0  # 追踪当前运动步数，用于检测策略是否完成
        ...
        with open(config_path, "r") as f:
            ...
            self.max_motion_steps = config.get("max_motion_steps", 906)
            policy_output
            ...
            print("BeyondMimicv1-like policy initializing ...")
    ...
    def run(self):
        ...
        motion_anchor_pos_b = np.zeros(3, dtype=np.float32)
        base_lin_vel = self.state_cmd.vel_cmd.astype(np.float32)
        ...
        # update motion phase
        self.counter_step += 1
        self.motion_step += 1  # 每个控制周期增加运动步数

    def exit(self):
        self.action = np.zeros(29, dtype=np.float32)
        self.action_buf = np.zeros(29 * self.history_length, dtype=np.float32)
        self.ref_motion_phase = 0.
    ...
    def checkChange(self):
        # Auto-transition to SkillCooldown when motion sequence completes
        if self.motion_step >= self.max_motion_steps:
            print(f"[BeyondMimic] Motion sequence completed ({self.motion_step} steps). "
                  f"Auto-switching to SkillCooldown for smooth transition...")
